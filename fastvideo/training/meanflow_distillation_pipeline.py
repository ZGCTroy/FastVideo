...
class MeanFlowDistillationPipeline(TrainingPipeline):
    """
    ...
    def _generator_forward(self, training_batch: TrainingBatch) -> torch.Tensor:
        ...
        return pred_noise, pred_video, noisy_latent
    ...
    def _generator_multi_step_simulation_forward(
        ...
        return pred_noise, pred_video, noisy_input

    def _get_unconditional_kwargs(self, input_kwargs: dict) -> dict:
        uncond_kwargs = input_kwargs.copy()
        
        if 'encoder_hidden_states' in uncond_kwargs:
            uncond_kwargs['encoder_hidden_states'] = torch.zeros_like(uncond_kwargs['encoder_hidden_states'])
            if 'encoder_attention_mask' in uncond_kwargs:
                uncond_kwargs['encoder_attention_mask'] = torch.zeros_like(uncond_kwargs['encoder_attention_mask'])
        else:
            raise ValueError("Cannot find conditioning key 'encoder_hidden_states' in input_kwargs for CFG.")
            
        return uncond_kwargs

    def _consistency_loss_forward(
        self,
        noisy_latent: torch.Tensor,
        generator_pred_noise: torch.Tensor,
        training_batch: TrainingBatch,
    ) -> torch.Tensor:
        """
        计算一致性损失
        1. 随机选择一个结束时间点 t_end < t_start。
        2. 如果 t_start 和 t_end 距离很近，使用带CFG的教师模型生成目标。
        3. 如果 t_start 和 t_end 距离较远，使用学生模型自己走两步来生成目标（自蒸馏）。
        """

        device = noisy_latent.device
        start_timestep = training_batch.timesteps # 注意一下这里的timesteps不是一个int值，会不会有问题
        input_kwargs = training_batch.input_kwargs
        start_timestep_val = start_timestep.item()

        # 1. 随机选择一个结束时间点 t_end，其中 0 <= t_end < t_start
        end_timestep_val = torch.randint(0, int(start_timestep_val), (1,)).item()
        end_timestep = torch.tensor([end_timestep_val], device=device, dtype=torch.long)

        # 2. 计算一致性目标 (Consistency Target)
        with torch.no_grad():
            # --- Self-Correction Path (two-step) ---
            # a. 随机选择一个中间时间步 t_mid，其中 t_end < t_mid < t_start
            mid_timestep_val = torch.randint(
                end_timestep_val + 1, int(start_timestep_val), (1,)
            ).long().item()
            mid_timestep = torch.tensor([mid_timestep_val], device=device, dtype=torch.long)
            
            distill_model = self.transformer

            # b. 预测 t_start -> t_mid
            kwargs_step1 = input_kwargs.copy()
            pred_noise_1 = distill_model(**kwargs_step1).permute(0, 2, 1, 3, 4)

            # c. 使用调度器从 t_start 前进到 t_mid
            latents_at_mid = self.noise_scheduler.step(
                pred_noise_1.flatten(0, 1),
                start_timestep.item(),
                noisy_latent.flatten(0, 1)
            ).prev_sample.unflatten(0, noisy_latent.shape[:2])

            # d. 预测 t_mid -> t_end
            kwargs_step2 = input_kwargs.copy()
            kwargs_step2['hidden_states'] = latents_at_mid.permute(0, 2, 1, 3, 4)
            kwargs_step2['timestep'] = mid_timestep
            pred_noise_2 = distill_model(**kwargs_step2).permute(0, 2, 1, 3, 4)

            # f. 根据时间步长的比例组合两个 x0 预测
            t_s, t_m, t_e = start_timestep.item(), mid_timestep.item(), end_timestep.item()
            # 避免除以零
            if (t_s - t_e) == 0:
                ratio1, ratio2 = 0.5, 0.5
            else:
                ratio1 = (t_s - t_m) / (t_s - t_e)
                ratio2 = (t_m - t_e) / (t_s - t_e)
            
            consistency_target = ratio1 * pred_noise_1 + ratio2 * pred_noise_2

        consistency_loss = F.mse_loss(generator_pred_noise.float(), consistency_target.float())
    ...
    def faker_score_forward(
        ...
        with torch.no_grad(), set_forward_context(
            ...
            if self.training_args.simulate_generator_forward:
                _, generator_pred_video, _ = self._generator_multi_step_simulation_forward(
                    training_batch)
            else:
                _, generator_pred_video, _ = self._generator_forward(training_batch)
        ...
        if (self.current_trainstep % self.generator_update_interval == 0):
            for batch in batches:
                ...
                with set_forward_context(
                    ...
                    if self.training_args.simulate_generator_forward:
                        generator_pred_noise, generator_pred_video, noisy_latent = self._generator_multi_step_simulation_forward(
                            batch_gen)
                    else:
                        generator_pred_noise, generator_pred_video, noisy_latent = self._generator_forward(
                            batch_gen)
                ...
                with set_forward_context(current_timestep=batch_gen.timesteps,
                                        attn_metadata=batch_gen.attn_metadata):
                    consistency_loss = self._consistency_loss_forward(
                        noisy_latent=noisy_latent,
                        generator_pred_noise=generator_pred_noise,
                        training_batch=batch_gen,
                    )

                # --- 组合DMD+CM损失并反向传播 ---
                gen_loss = dmd_loss + consistency_loss

                with set_forward_context(
                        current_timestep=batch_gen.timesteps,
                        attn_metadata=batch_gen.attn_metadata_vsa):
                    (gen_loss / gradient_accumulation_steps).backward()
                total_dmd_loss += dmd_loss.detach().item()
                total_cm_loss += consistency_loss.detach().item()
                total_gen_loss += gen_loss.detach().item()
            ...
            avg_dmd_loss = torch.tensor(total_dmd_loss /
                                        ...
                                        device=self.device)
            avg_cm_loss = torch.tensor(total_cm_loss /
                                        gradient_accumulation_steps,
                                        device=self.device)
            avg_gen_loss = torch.tensor(total_gen_loss /
                                        gradient_accumulation_steps,
                                        device=self.device)
            world_group = get_world_group()
            world_group.all_reduce(avg_dmd_loss,
                                   op=torch.distributed.ReduceOp.AVG)
            world_group.all_reduce(avg_cm_loss,
                                   op=torch.distributed.ReduceOp.AVG)
            world_group.all_reduce(avg_gen_loss,
                                   op=torch.distributed.ReduceOp.AVG)
            training_batch.generator_loss = avg_gen_loss.item()
            training_batch.consistency_loss = avg_cm_loss.item()
            training_batch.dmd_loss = avg_dmd_loss.item()
            dmd_latent_vis_dict = batch_gen.dmd_latent_vis_dict
        else:
            training_batch.generator_loss = 0.0
            training_batch.consistency_loss = 0.0
            training_batch.dmd_loss = 0.0
        ...
        for step in range(self.init_steps + 1,
            ...
            if self.global_rank == 0:
                ...
                if (step % self.generator_update_interval == 0):
                    ...
                    log_data["train_consistency_loss"] = training_batch.consistency_loss
                    log_data["train_dmd_loss"] = training_batch.dmd_loss
                if use_vsa:
